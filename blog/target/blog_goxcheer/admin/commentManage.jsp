...
<script type="text/javascript">
	...
	function deleteComment(){
		...
		for(var i=0;i<selectedRows.length;i++){
			strIds.push({"id":selectedRows[i].id,"blogId":selectedRows[i].blog.id,"state":selectedRows[i].state});
		}
		alert(strIds[0].id+","+strIds[0].blogId);
		$.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/admin/comment/delete.do",{ids:JSON.stringify(strIds)},function(result){
					if(result.success){
