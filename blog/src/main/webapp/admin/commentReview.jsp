...
<script type="text/javascript">
 	...
 	function commentReview(state){
		...
		for(var i=0;i<selectedRows.length;i++){
			strIds.push(new comObject(selectedRows[i].id,selectedRows[i].blog.id));
		}
		$.messager.confirm("系统提示","您确定要审核这<font color=red>"+selectedRows.length+"</font>条评论吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/admin/comment/review.do",{ids:JSON.stringify(strIds),state:state},
					function(result){
					if(result.success){
